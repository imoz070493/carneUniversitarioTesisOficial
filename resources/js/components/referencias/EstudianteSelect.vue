<template>
  <div>
    <v-select
      v-model="lugar"
      label="data"
      :options="array_estudiantes"
      placeholder="Buscar Estudiante..."
      :clearable="true"
      :disabled="disabled"
      @search="selectEstudiante"
      @input="getDatosEstudiante"
    >
      <span slot="no-options">No se encontraron resultados.</span>
      <!-- <template slot="option" slot-scope="option">{{ option.data }}</template> -->
      <template #option="{ data, codigo_estudiante, apellido_paterno, apellido_materno, nombres }">
        <em>{{ data }}</em>
      </template>
      <template slot="selected-option" slot-scope="option">{{option.data}}</template>
    </v-select>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    selectEstudiante(search) {
      let me = this;
      if(!search)
        return;

      let url = '/estudiante/select?buscar='+search;
