<template>
  <div>
    ...
    >
      ...
      <template #option="{ data, nombre_proveedor }">
        <em>{{ data }} {{ nombre_proveedor }}</em>
      </template>
      <template slot="selected-option" slot-scope="option">{{option.nombre_proveedor}}</template>
    </v-select>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    selectPersona(search) {
      ...
      let url = '/proveedor/select?buscar='+search;

      axios.get(url).then(function (response){
          me.array_proveedores = response.data;
      })
