                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <table class="table table-bordered table-striped table-sm">
                            ...
                            <tbody>
                                <tr v-if="show.arrayMarca"><th colspan="4" class="text-center text-dark" v-text="'Vacio'"></th></tr>
                                <tr v-for="marca in arrayMarca" :key="marca.id">
                                    <td>
                                        <button type="button" @click="editar(marca)" class="btn btn-warning btn-sm">
                                          <i class="icon-pencil"></i>
                                        </button> &nbsp;
                                        <template v-if="marca.estado=='activo'">
                                            <button type="button" class="btn btn-danger btn-sm" @click="desactivarMarca(marca.id)">
                                            <i class="icon-trash"></i>
                                            </button>
                                        </template>
                                        <template v-else-if="marca.estado=='inactivo'">
                                    ...
                                    <td>
                                        <div v-if="marca.estado=='activo'">
                                            <span class="badge badge-success">Activo</span>
                                        </div>
                                        <div v-else-if="marca.estado=='inactivo'">
...
const FormularioMarca = () => import("@/components/erp/marca/FormularioMarca");

export default {
    components: {
        "v-formulario-marca": FormularioMarca,
    },
    data(){
        return {
            nuevo: {},
    ...
    methods: {
        listarMarca(page, buscar, criterio){
            let me = this;
            var url = '/marca?page='+page+'&buscar='+buscar+'&criterio='+criterio+'&per_page='+this.per_page;
            axios.get(url).then(function (response){
                var respuesta = response.data;
                me.arrayMarca = respuesta.marcas.data;
                me.pagination = respuesta.pagination;

                if(me.arrayMarca.length==0) me.show['arrayMarca'] = true;
                else me.show['arrayMarca'] = false;
            })
