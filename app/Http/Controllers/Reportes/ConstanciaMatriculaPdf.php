...
class ConstanciaMatriculaPdf
{
    public static function buildReport($datos)
    {
        ...
        $pdf->SetXY(20,60);
        $fontname = TCPDF_FONTS::addTTFfont(public_path().'/fonts/BELLB.TTF', 'TrueTypeUnicode', '', 96);
        $pdf->SetFont($fontname, '', 20, '', false);
        // $pdf->SetFont('times', '', 20, '', false);
        $pdf->MultiCell(170, 20, "CONSTANCIA DE MATRICULA", '', 'C', 0, 1, '', '', true, 0, false, true, 20, 'M');
        
        $fontname = TCPDF_FONTS::addTTFfont(public_path().'/fonts/ariali.ttf', 'TrueTypeUnicode', '', 96);
        $pdf->SetFont($fontname, '', 12, '', false);
        $pdf->SetX(25);
        $la_oficina_de_registros = "La Oficina de Registros Académicos de la Universidad Nacional José María Arguedas.";
        $altura_a = $pdf->GetStringHeight(160, $la_oficina_de_registros) + 0.5;
        $pdf->MultiCell(160, $altura_a, $la_oficina_de_registros, '', 'J', 0, 0, '', '', true, 0, false, true, $altura_a, 'M');
        $pdf->Ln(13);

        $fontname = TCPDF_FONTS::addTTFfont(public_path().'/fonts/arialbi.ttf', 'TrueTypeUnicode', '', 96);
        $pdf->SetFont($fontname, '', 12, '', false);
        $pdf->SetX(25);
        $altura_b = $pdf->GetStringHeight(160, "Hace constar:") + 0.5;
        $pdf->MultiCell(160, $altura_b, "Hace constar:", '', 'L', 0, 0, '', '', true, 0, false, true, $altura_b, 'M');
        $pdf->Ln(8);

        $nombre_completo = trim($datos['data']->apellido_paterno)." ".trim($datos['data']->apellido_materno).", ".trim($datos['data']->nombres);
        $dni = $datos['data']->dni;
        $codigo_estudiante = $datos['data']->codigo_estudiante;
        $escuela_profesional = $datos['data']->escuela_profesional;

        if( $escuela_profesional=='ADMINISTRACIÓN DE EMPRESAS' ||
            ...
            $escuela_profesional=='PSICOLOGÍA'
        ){
            $sede = 'Ccoyahuacho';
        ...
        if($datos['data']->estado_periodo=='Finalizado')
            $body="Que el(la) estudiante: $nombre_completo; identificado con DNI: $dni y código: $codigo_estudiante, perteneciente a la Escuela Profesional de $escuela_profesional, con Sede - $sede, ha registrado matricula en el $ciclo Ciclo, en el semestre académico $periodo, así consta en el Sistema Integrado de Gestión Académica Universitaria (SIGAU – UNAJMA).<br>Se expide la presente constancia, a solicitud $de_la, para los fines que estime pertinente.";
        if($datos['data']->estado_periodo=='Activo')
            $body="Que el(la) estudiante: $nombre_completo; identificado con DNI: $dni y código: $codigo_estudiante, perteneciente a la Escuela Profesional de $escuela_profesional, con Sede - $sede, registra matricula en el $ciclo Ciclo, en el semestre académico $periodo, así consta en el Sistema Integrado de Gestión Académica Universitaria (SIGAU – UNAJMA).<br>Se expide la presente constancia, a solicitud $de_la, para los fines que estime pertinente.";

        $fontname = TCPDF_FONTS::addTTFfont(public_path().'/fonts/ariali.ttf', 'TrueTypeUnicode', '', 96);
        $pdf->SetFont($fontname, '', 12, '', false);
        $pdf->SetX(25);
        $pdf->setCellHeightRatio(2);//Interlineado
        $altura = $pdf->GetStringHeight(160, $body) + 0.5;
        $pdf->MultiCell(160, $altura, $body, '', 'J', 0, 1, '', '', true, 0, true, true, $altura, 'M');

        $nota = "Nota: La Escuela Profesional de $escuela_profesional consta de 10 ciclos académicos.";
        $fontname = TCPDF_FONTS::addTTFfont(public_path().'/fonts/ariali.ttf', 'TrueTypeUnicode', '', 96);
        $pdf->SetFont($fontname, '', 9, '', false);
        $pdf->SetX(25);
        $pdf->setCellHeightRatio(2);//Interlineado
        $altura_c = $pdf->GetStringHeight(160, $nota) + 0.5;
        $pdf->MultiCell(160, $altura_c, $nota, '', 'J', 0, 0, '', '', true, 0, true, true, $altura_c, 'M');
        $pdf->Ln(13);

        $fecha_leida = static::getFechaLeida($datos['data']->fecha_procesado);
        $fontname = TCPDF_FONTS::addTTFfont(public_path().'/fonts/ariali.ttf', 'TrueTypeUnicode', '', 96);
        $pdf->SetFont($fontname, '', 12, '', false);
        $pdf->SetX(25);
        $pdf->setCellHeightRatio(2);//Interlineado
