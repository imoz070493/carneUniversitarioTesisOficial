...
class PhotosExport
{
	public static function exportExcelTable($data, $nombreTabla, $filename = false,$formateadores = [])
	{
		...
		foreach($data as $key => $value) {
			...
			$documento->getActiveSheet()->getRowDimension($row)->setRowHeight(120);
			$hoja->setCellValue("A" . $row, $key + 1);
			$hoja->setCellValue("B" . $row, $value->credencial_validado=='validado'?$value->nombres_validado:$value->nombres);
			$hoja->setCellValue("C" . $row, $value->credencial_validado=='validado'?$value->apellido_paterno_validado:$value->apellido_paterno);
			$hoja->setCellValue("D" . $row, $value->credencial_validado=='validado'?$value->apellido_materno_validado:$value->apellido_materno);
			$hoja->setCellValue("E" . $row, $value->codigo_estudiante);
            ...
            // foreach ($value->detalle_entrega as $key => $value) {
	...
	public static function setHeaderV2($documento){
		$hoja = $documento->getActiveSheet();
		...
		$hoja->setCellValue("E1", "CODIGO ESTUDIANTE");
		$hoja->setCellValue("F1", "ESCUELA PROFESIONAL");
		$hoja->setCellValue("G1", "DNI");
		$hoja->setCellValue("H1", "SEXO");
		$hoja->setCellValue("I1", "FOTO");
		...
		//Asigna estilos al encabezado
		$hoja->getStyle("A1:I1")->applyFromArray($styleArray)
					->getAlignment()
					->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER)
					->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER)
					->setWrapText(true);
